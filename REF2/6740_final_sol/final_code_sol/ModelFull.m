...
pos_train = -sum(((xtrain - repmat(pos_mean, size(train, 1), 1)) * V_pos).^2, 2)/2 ...
    - log(sum(S_pos))/2 + log(pi_pos);
neg_train = -sum(((xtrain - repmat(neg_mean, size(train, 1), 1)) * V_neg).^2, 2)/2 ...
    - log(sum(S_neg))/2 + log(pi_neg);

pos_test = -sum(((xtest - repmat(pos_mean, size(test, 1), 1)) * V_pos).^2, 2)/2 ...
    - log(sum(S_pos))/2 + log(pi_pos);
neg_test = -sum(((xtest - repmat(neg_mean, size(test, 1), 1)) * V_neg).^2, 2)/2 ...
    - log(sum(S_neg))/2 + log(pi_neg);
